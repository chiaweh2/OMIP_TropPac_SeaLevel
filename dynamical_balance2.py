...
def ekman_pumping2_3d(da_tauuo,da_tauvo,eq_mask=True):
    """
    Calculate Ekman pumping based on wind stress curl
    ...
    da_tauuo = da_tauuo/f/sea_density
    da_tauvo = da_tauvo/f/sea_density     # m^2/s
    da_ekw_u,da_ekw_v = curl_tau_3d(da_tauuo,da_tauvo) 
...
def curl_tau(da_tauuo,da_tauvo):
    """
    ...
    # calculate dy 
    dy = du_dy.lat.copy()+np.nan
    dyl = du_dy.lat.diff('y',1,label='lower')
    dyu = du_dy.lat.diff('y',1,label='upper')
    ...
    # calculate dx 
    dx = dv_dx.lon.copy()+np.nan
    dxl = dv_dx.lon.diff('x',1,label='lower')
    dxu = dv_dx.lon.diff('x',1,label='upper')
...
def curl_tau_3d(da_tauuo,da_tauvo):
    """
    Calculate wind stress curl 
